...
export const generateRandomPassword = (options: PassswordOptions): string => {
    ...
    return Array.from(passwordArray, (num) => charPool[num % charPool.length]).join("");
}
...
export const generatePassphrase = async (
    ...
    const passphraseWords = await Promise.all(Array.from({ length: wordCount }, () => getRandomWord()));

    // Add optional numbers and symbols
    if(includeNumbers) passphraseWords.push(crypto.randomInt(0, 10).toString());

    if(includeSymbols) passphraseWords.push(SYMBOLS[crypto.randomInt(0, SYMBOLS.length - 1)]);
